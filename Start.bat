...
echo 正在启动割草机服务端...
timeout 3
start /high ..\运行环境\bin\java.exe -jar ..\割草机\5.0\gc.jar
timeout 120
echo 已尝试启动服务端。
echo ------------------------------
cd ..
...
echo 1.确保LeafGS的主文件夹(GSlauncher)与原神的主文件夹(genshin impact game)在同一目录下。
...
color 67
title 重启中
echo 成功将"%select2%"设为原神主程序路径，LeafGS将在3秒后自动重启！
timeout 3 >nul
goto restart